...
export class ProgressBarComponent implements OnInit, AfterViewInit, OnChanges {
    ...
    ngOnChanges(changes: SimpleChanges): void {
        ...
        if (
            (changes.progressUpdateNotifierValue &&
                changes.progressUpdateNotifierValue.currentValue !== changes.progressUpdateNotifierValue.previousValue) ||
            (changes.isProcessFailed && changes.isProcessFailed.currentValue !== changes.isProcessFailed.previousValue)
        ) {
    ...
    increamentProgressBar(): void {
        if (this.progressBreadCrumbEl) {
            const stepClasses = this.progressBreadCrumbEl.nativeElement.querySelectorAll('.step');
